#!/bin/bash
# Rollout script for avatar_url_2 generation for all uncensored MALE companions
# Runs the bulk script repeatedly until all companions have avatar_url_2

FUNCTION_URL="https://selira.ai/.netlify/functions/generate-avatar-2-uncensored-male-bulk?trigger=manual"
MAX_ITERATIONS=200  # Safety limit
DELAY_BETWEEN_CALLS=35  # 35 seconds between calls (function takes ~20s + buffer)

echo "🚀 Starting Uncensored Male Avatar_url_2 Rollout"
echo "=================================================="
echo ""
echo "Target: All Selira uncensored MALE companions without avatar_url_2"
echo "Expected: ~87 companions to process"
echo "Time per companion: ~35 seconds (20s generation + 15s buffer)"
echo "Estimated total time: ~1 hour"
echo ""
