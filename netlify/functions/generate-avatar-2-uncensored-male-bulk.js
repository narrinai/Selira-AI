// netlify/functions/generate-avatar-2-uncensored-male-bulk.js
// Scheduled function to generate avatar_url_2 for UNCENSORED MALE companions that don't have one yet
  ...
  try {
    ...
    const response = await fetch('https://selira.ai/.netlify/functions/selira-generate-custom-image', {
      ...
      body: JSON.stringify({
        ...
        source: 'avatar-2-uncensored-male-bulk',
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    console.log('📊 Fetching UNCENSORED MALE Selira companions without Avatar_URL_2...');

    let allCompanions = [];
    let offset = null;

    do {
      // Filter: No Avatar_URL_2 AND Created_by = 'Selira' AND content_filter = 'Uncensored' AND sex = 'male'
      const filterFormula = encodeURIComponent("AND(NOT({Avatar_URL_2}), {Created_by} = 'Selira', {content_filter} = 'Uncensored', {sex} = 'male')");
