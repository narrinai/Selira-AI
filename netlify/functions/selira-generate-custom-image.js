...
async function generateWithPromptchan(body, requestId, corsHeaders, email, auth0_id) {
  ...
  if (false) { // DEAD CODE - keeping for reference but never executes
    ...
    let promptchanModelStyle = 'Hyperreal';  // Default realistic (faster than XL+ v2)
    if (style === 'anime' || style === 'animated') {
      promptchanModelStyle = 'Anime XL+';
      console.log(`🎌 [${requestId}] Using Anime XL+ model for anime companion`);
    } else {
      console.log(`📸 [${requestId}] Using Hyperreal model for realistic companion`);
    }
    ...
    const promptchanRequest = {
      ...
      creativity: 10, // Low = faster generation (was 30, then 5)
      seed: -1,
      filter: 'Default',
      emotion: 'Default',
      detail: 0,
  ...
  if (isUncensoredExplicit) {
    ...
    const promptchanRequest = {
      ...
      creativity: 10, // Low = faster generation
      seed: -1,
      filter: 'Default',
      emotion: 'Default',
      detail: 0,
  ...
  let promptchanStyle = 'Hyperreal';  // Use Hyperreal for realistic (faster than XL+ v2)
