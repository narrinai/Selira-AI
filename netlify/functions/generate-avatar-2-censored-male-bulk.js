// netlify/functions/generate-avatar-2-censored-male-bulk.js
// Scheduled function to generate avatar_url_2 for CENSORED MALE companions that don't have one yet
...
async function generateAvatar2(companion) {
  ...
  // Variation prompts - CENSORED tasteful poses (no nudity, no explicit content)
  const variations = companion.sex === 'male' ? [
    'professional portrait, handsome man sitting confidently, wearing stylish casual outfit, looking at camera, full body visible, charismatic pose, natural lighting',
    'elegant shot, attractive man standing with arms crossed, dressed in sophisticated attire, confident expression, showing entire body, professional photography',
    'artistic portrait, fit man leaning against wall, wearing fashionable clothes, alluring gaze, complete figure visible, tasteful composition'
  ] : [
    // FEMALE: 4 rotating tasteful poses - NO NUDITY, elegant and classy
    'beautiful woman sitting elegantly, wearing stylish dress, looking over shoulder, professional portrait, tasteful pose, graceful posture, single person only',
    'gorgeous woman standing confidently, fashionable outfit, one hand on hip, alluring smile, full body visible, professional photography, elegant composition',
    'stunning woman reclining gracefully, wearing elegant attire, soft lighting, artistic portrait, sophisticated pose, tasteful angle, classy atmosphere',
    'attractive woman posing dynamically, stylish clothing, confident expression, professional shot, full figure visible, fashionable portrait, single person only'
  ];
  ...
  try {
    ...
    const response = await fetch('https://selira.ai/.netlify/functions/selira-generate-custom-image', {
      ...
      body: JSON.stringify({
        ...
        sex: companion.sex || 'male',
        ethnicity: companion.ethnicity || 'white',
        hairLength: companion.hair_length || 'short',
        hairColor: companion.hair_color || 'brown',
        source: 'avatar-2-censored-male-bulk',
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    console.log('📊 Fetching CENSORED MALE Selira companions without Avatar_URL_2...');

    let allCompanions = [];
    let offset = null;

    do {
      // Filter: No Avatar_URL_2 AND Created_by = 'Selira' AND content_filter = 'Censored' AND sex = 'male'
      const filterFormula = encodeURIComponent("AND(NOT({Avatar_URL_2}), {Created_by} = 'Selira', {content_filter} = 'Censored', {sex} = 'male')");
